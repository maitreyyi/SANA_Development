...
import { z } from "zod";
import { modelOptionsSchemas, SanaModelType, SanaOptions } from "../config/modelOptions";
import HttpError from "../middlewares/HttpError";
 ...
 */
export const sanitize = async <T extends SanaOptions>(
  options: T,
  version: SanaModelType
): Promise<T> => {
  try {
    // Get the appropriate schema for the model version
    const schema = modelOptionsSchemas[version];
    ...
    if (error instanceof z.ZodError) {
      ...
      throw new HttpError(`Invalid options: ${formattedErrors}`, { status: 400 });
    }
    throw error;
  }
};
